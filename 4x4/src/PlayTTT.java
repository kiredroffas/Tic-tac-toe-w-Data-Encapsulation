...
public class PlayTTT {
  ...
  public static void main(String[] args) {
    ...
    while(cont == 0) {
      ...
      System.out.println("Welcome to 4x4 Tic-Tac-Toe, try to get 3 in a row! Would you like to go first? y/n");
      ...
      while (newGame.getStatus() != 1 && newGame.getStatus() != 2 && newGame.getStatus() != 3) {
        if (token == 1) {  //User turn
          System.out.println("Enter the coordinates you'd like to place a token at (separated by comma [0-3] [0-3], i.e. 0,2 = [0, 2]) or type 'pm' to place a mark for you.");
          ...
          if (input.charAt(0) == 'p' && input.charAt(1) == 'm') {  //If user wants to have a move made for them
            ...
            while (newGame.placeMark(xcoord, ycoord, 1) == false) {
              System.out.println("Invalid marker coordinates! Enter new coordinates (separated by comma [0-3] [0-3], i.e. 0,2 = [0, 2]):");
              newGame.printGrid();
              input = sc.nextLine();
              xcoord = Character.getNumericValue(input.charAt(0));
              ycoord = Character.getNumericValue(input.charAt(2));
